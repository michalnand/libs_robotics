...
w_nom       = 310.0*2.0*torch.pi/60.0       #no load speed,         310 or 1000[rpm]
torque      = 1.7*0.09807                   #stall torque,          1.7 or 0.57[kg.cm]
...
controller              = LibsControll.LinearQuadraticControllerHidden(required_dim, plant_outputs, plant_inputs, 2)
...
for i in range(200):
    ...
    if i%10 == 0:
        ...
        LibsControll.plot_controll_output(time_trajectory, controller_u_trajectory, required_trajectory, plant_y_trajectory, ["voltage [V]"], ["speed [degrees/s]", "position [degrees]"], "imgs/lqch_servo/" + str(i))
