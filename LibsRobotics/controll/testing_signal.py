...
class UnitStepSignal:
    def __init__(self, samples_count, channels_count, period = 5, randomise = False, amp_max = 1.0):
    ...
    def sample_batch(self, batch_size = 32, batch_first = False):
        if self.randomise:
            amplitudes  = self.amp_max*(2.0*numpy.random.rand(batch_size, self.channels_count) - 1.0)
            phases      = numpy.random.randint(0, int(self.samples_count*0.8), (batch_size, self.channels_count))
        else:
            amplitudes  = self.amp_max*numpy.ones((batch_size, self.channels_count))
